...
Kalau Anda memakai sistem build lain, langkah ini boleh dilewati — asalkan sistem itu menghasilkan berkas JSON yang sah sesuai [spesifikasi skema registry-item](/registry/registry-item-json).
...
Berkas `registry.json` ini harus sesuai [spesifikasi skema registry](/registry/registry-json).
...
Penjelasan lengkap soal skema item registry dan jenis berkasnya ada di [dokumentasi skema registry item](/registry/registry-item-json).
...
Folder tujuannya bisa diubah lewat opsi `--output`. Keterangan lengkapnya ada di [perintah build](/cli#build).
:::
